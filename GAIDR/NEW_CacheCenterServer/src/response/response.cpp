...
double response(string filename, int sx, int sy, int ex, int ey, int x0, int y0, string path){
	...
	if(!fits_open_file(&infptr, infilename.c_str(), READONLY, &status)){
	        ...
	        string outfilename = TMP_TARGET_PATH + filename + s2;
...
double extractFile2Replica(string filename, int sx, int sy, int ex,int ey){
	...
	string s1 = instrStream.str();
	string infilename = ORIGINAL_SOURCE1_PATH + filename + s1;

	if(!fits_open_file(&infptr, infilename.c_str(), READONLY, &status)){
		stringstream outstrStream;
		outstrStream<<"_"<<sx<<"_"<<sy<<"_"<<ex<<"_"<<ey<<".fits";
		string s2 = outstrStream.str();
		string outfilename = TMP_RELICA_PATH + filename +s2;
		cout<<"outfilenameoutfilenameoutfilenameoutfilename"<<outfilename<<endl;
	...
	gettimeofday(&end, NULL);
	duration = (end.tv_sec-start.tv_sec)*1000 + (end.tv_usec-end.tv_usec)/1000.0;
	return duration;
}
